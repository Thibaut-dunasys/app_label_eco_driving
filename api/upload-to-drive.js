...
export default function DrivingLabelingApp() {
  ...
  // Gestion de la centrale inertielle
  useEffect(() => {
    if (!isRunning) return;
  ...
  // Enregistrer les données IMU toutes les secondes
  useEffect(() => {
  ...
  const toggleLabel = (labelId) => {
    ...
    // Ajouter "Initialisation" si c'est le premier label
    ...
    if (activeLabels[labelId]) {
      ...
      // Fermer tous les labels actifs avant d'activer le nouveau
  ...
  const endSession = () => {
    ...
    if (finalRecordings.length === 0 && Object.keys(activeLabels).length === 0) {
      ...
      Object.keys(activeLabels).forEach(labelId => {
        finalRecordings.push({
          label: labels.find(l => l.id === labelId).name,
          startTime: formatTime(activeLabels[labelId]),
          endTime: formatTime(currentTime),
          duration: formatTime(currentTime - activeLabels[labelId]),
          absoluteStartTime: new Date(sessionStartDate.getTime() + activeLabels[labelId]),
          absoluteEndTime: endDate
        });
      });
    }
  ...
  const uploadToDrive = async (data, session) => {
    ...
    try {
      ...
      // Fallback: télécharger localement
